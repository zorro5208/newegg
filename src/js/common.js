...
//限时购
function shopping(times,tag){//格式'2019-4-22 16:44:00'
	//1.设置终点时间
	var endTime = times;
	var end = Date.parse(endTime); //转换秒数
	
	/*2.不断获取系统当前的时间(开定时器)，终点时间 - 当前时间  <= 0 秒
	 * 关闭定时器
	 * 替换按钮和商品的图片*/
	function time() {
		var nowTime = Date.now(); //获取当前时间,并自动转化秒数
		var time = parseInt((end - nowTime) / 1000); //临界时间
		if(time <= 0) {
			//临界时间为0，可以购买
			...
			var t = day(time);
			var html = `剩余时间:&nbsp; &nbsp;${t.ds}天${t.hs}时${t.ms}分${t.ss}秒`;
			tag.innerHTML = html;
		}
	}
	time();
	var timer = setInterval(time, 1000); //开启定时器
	//3.毫秒数->xx天xx时xx分xx秒
	function day(time) {
